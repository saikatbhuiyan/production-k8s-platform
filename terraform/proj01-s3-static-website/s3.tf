...
resource "aws_s3_bucket" "static_website" {
  bucket = "terraform-s3-static-website-${random_id.bucket_suffix.hex}"
}

resource "aws_s3_bucket_public_access_block" "static_website" {
  bucket                  = aws_s3_bucket.static_website.id
  # Disable S3 public access blocks so the bucket policy can expose the static site publicly.
